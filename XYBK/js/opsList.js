	...
	for (var i = 0; i < opsList.length; i++) {
		var UL = $('<ul class="zj-ops-piece"><li id="'+opsList[i][field].toLowerCase()+'">'+opsList[i][field].toUpperCase()+'</li></ul>');
			for (var j = 0; j < opsList[i].list.length; j++) {
				$('<li data-href="'+opsList[i].list[j][field3]+'">'+opsList[i].list[j][field2]+'</li>').appendTo(UL).data("data",opsList[i].list[j]);
			}
...
function paramList (data,theVal,field,field2,field3,parentDom){
	...
	for(var i = 0; i < data.length; i++){
		if(theVal.toLowerCase() == data[i][field].substr(0,1) || theVal.toUpperCase() == data[i][field].substr(0,1) || theVal == data[i][field].substr(0,1) || theVal == data[i][field2].substr(0,1)){
			...
			emptyFlag = false;
			$('<li data-href="'+data[i][field3]+'">'+data[i][field2]+'</li>').appendTo(UL).data("data",data[i]);
		}
	}
	if(emptyFlag){
...
/*----------【以下为本页所需js逻辑】----这是一条华丽的分割线-----------------------------*/
	...
	$(function(){
		...
		//testData 为动态取到的数据
		//通过首字母正序排列【动态取到的数据】；
		var theData = testData.resultList.zjListSort({"fieldType":"chinese","field":"ENGCODE"});
		console.log(theData);
		//去重后获取的数据
		var newOpsListData = newOpsList(theData,"ENGCODE");
		//调用列表渲染函数;
		...
		//【event】搜索框点击事件挂载
		searchBox.on("focusin click",function(){
			$(this).css({
				"width":"87%"
			});
			searchCancel.css({
		...
		opsList.parent().on('click','li',function(e){
			...
			if(e.target.getAttribute("data-href") == null || e.target.getAttribute("data-href") == 'null' || e.target.getAttribute("data-href") == ""){
				...
				console.dir($(e.target).data("data"))
				return false;
			}
		});
		//【event】输入框搜索匹配事件
		searchInput.on("input propertychange",function(){
			...
			if(val != ""){
				opsList.hide();
				targetBox.hide();
				paramList (theData,val,'ENGCODE','CITYNAME','BRANCHURL','.zj-search-resulit');
			}else{
				opsList.show();
				targetBox.show();
				searchResulit.empty();
